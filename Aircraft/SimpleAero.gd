...
@export var alignment_strength: float = 1.75   # High-speed horizontal sideslip damping in 1/s (mass-scaled below)
@export var alignment_low_speed_strength: float = 0.4   # Moderate low-speed horizontal alignment for tighter slip damping
...
@export var vertical_alignment_low_speed_strength: float = 0.008  # Keep low-speed vertical path alignment essentially absent so pitch still matters
@export var vertical_alignment_high_speed_strength: float = 0.55  # Moderate vertical path alignment at speed so fast flight feels planted without becoming sticky
