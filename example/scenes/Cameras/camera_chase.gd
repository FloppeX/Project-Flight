...
func update_position(delta):
	...
	# Smooth rotation to look at aircraft
	var target_transform = transform.looking_at(aircraft_pos, Vector3.UP)
	rotation = rotation.lerp(target_transform.basis.get_euler(), rotation_smoothing * delta)
