...
#### Per-Frame Carrier Tracking (`LandCarrier/LandCarrier.gd` — `_carry_deck_passengers`)
...
- Added `[Deck]` debug print every 2s showing plane Z, carrier Z, and gap — auto-stops when `controls_disabled` is cleared at launch.
