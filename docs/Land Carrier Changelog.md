...
- 2026-09-04: Carrier hold/launch authority fix - aircraft launch constraints no longer impose their 8 m/s straight-deck speed or request an autonomous terrain-clearance reposition when the carrier has no active navigation order. `HOLD` also cancels an in-progress launch reposition; launches from a clear stationary heading remain allowed, while a terrain-blocked launch waits for the player to move the carrier.
- 2026-09-04: Tactical-map navigation pass - added cursor-centred `1x`-to-`8x` zoom using right trigger/left mouse to zoom in and left trigger/right mouse to zoom out, plus horizontal and vertical scrollbars for panning. Terrain, fog, mobility, symbols, routes, grid coordinates, hover data, and command hit-testing now use one shared view transform; order-drafting clicks preserve their existing target and waypoint-editing behavior.
