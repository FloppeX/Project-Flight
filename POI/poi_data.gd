...
## Optional world-space site scene. The POI manager owns its placement and the
## scene owns only presentation/collision.
@export var world_scene: PackedScene = null
## Small effect identifier resolved by POIManager when the player makes a
## decision. Keeping this data-driven lets later cards share effect handlers.
