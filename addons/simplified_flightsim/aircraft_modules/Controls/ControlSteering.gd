...
@export var rudder_assist_max_input: float = 0.35
...
@export var rudder_assist_lateral_g_weight: float = 0.0
...
func _apply_rudder_assist(manual_yaw: float, delta: float) -> float:
	...
	if advanced_fixed_wing:
		...
		max_input = lerpf(
			max_input,
			clampf(rudder_assist_stiffened_max_input, 0.0, max_input),
	...
	var assisted_yaw := clampf(manual_yaw + _filtered_assist_yaw * manual_fade, -1.0, 1.0)
...
func _estimate_slip_ball_error(delta: float, use_helicopter_tuning: bool) -> float:
	...
	# SimpleAero already applies a lateral alignment acceleration. Feeding that
	# acceleration back as the primary slip signal made the assist react to its
	# own correction and alternate full rudder after surface-rate lag was added.
	# Fixed-wing assistance therefore follows geometric sideslip, with an
	# optional small force term kept only as an airframe-tuning hook.
	return _combine_fixed_wing_slip_error(velocity_slip, force_slip)
...
func _combine_fixed_wing_slip_error(velocity_slip: float, force_slip: float) -> float:
	return clampf(
		velocity_slip * maxf(rudder_assist_velocity_slip_weight, 0.0)
			+ force_slip * maxf(rudder_assist_lateral_g_weight, 0.0),
		-1.0,
		1.0
	)
...
func _get_fixed_wing_rudder_stiffening() -> float:
	if not _simple_aero_has_control_envelope \
		...
		return 0.0
	return clampf(float(simple_aero.get("current_high_speed_stiffening")), 0.0, 1.0)
