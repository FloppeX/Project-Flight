...
func fire() -> bool:
	if not can_fire():
		print("[AGMissileHolder] Cannot fire: ammo=", ammo_count, " cooldown=", fire_cooldown)
		return false
	...
	if missile and missile.has_method("fire_with_target"):
		...
		print("[AGMissileHolder] WARNING: Missile instance has no fire methods")
	
	# Trigger missile camera tracking on instrument panel
	_start_missile_camera_tracking(missile, aircraft)
	
	ammo_count -= 1
	print("[AGMissileHolder] Fired missile. Remaining ammo=", ammo_count)
...
func _start_missile_camera_tracking(missile: Node3D, aircraft: Node3D) -> void:
	...
	if instrument_panel and instrument_panel.has_method("start_missile_camera_tracking"):
		print("[AGMissileHolder] Starting missile camera tracking")
		instrument_panel.start_missile_camera_tracking(missile)
	else:
		print("[AGMissileHolder] Could not find instrument panel for missile camera")
...
func _ensure_missile_scene() -> bool:
	...
	for p in candidates:
		if ResourceLoader.exists(p):
			...
			if s:
				missile_scene = s
				print("[AGMissileHolder] Loaded missile scene: ", p)
				return true
	print("[AGMissileHolder] ERROR: No valid missile scene found in candidates")
	return false
