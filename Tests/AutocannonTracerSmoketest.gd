...
func _run() -> void:
	_check_profile(PROFILE_10, 0.2, 0.8, 2, 800.0 / 60.0 * 5.0, "10 mm")
	_check_profile(PROFILE_15, 0.2, 0.8, 2, 600.0 / 60.0 * 10.0, "15 mm")
	_check_profile(PROFILE_20, 0.28, 1.1, 1, 400.0 / 60.0 * 20.0, "20 mm")
	_check_profile(PROFILE_25, 0.34, 1.35, 1, 300.0 / 60.0 * 30.0, "25 mm")
	_check_profile(PROFILE_40, 0.44, 1.75, 1, 120.0 / 60.0 * 42.0, "40 mm")
	...
	if round != null:
		...
		_expect(round.tracer_width >= 0.28, "explosive autocannon fallback tracer remained too narrow")
		_expect(round.tracer_visual_length <= round.tracer_width * 4.0, "explosive autocannon fallback tracer remained line-like")
	...
	await get_tree().process_frame
	_expect(is_equal_approx(aircraft_1_gun.physical_tracer_length_m, 0.8), "Aircraft 1 physical tracer remained a four-metre line")
	_expect(is_equal_approx(aircraft_1_gun.virtual_tracer_length_m, 0.8), "Aircraft 1 virtual tracer remained a four-metre line")
...
func _check_profile(
	...
	_expect(profile.physical_tracer_length_m <= profile.physical_tracer_width_m * 4.0, "%s tracer aspect ratio remained line-like" % label)
...
func _check_tracer_geometry_and_material() -> void:
	...
	if mesh != null and mesh.get_surface_count() == 1:
		...
		_expect(vertices.size() == 14, "shared tracer mesh lost its radial base, tip, or cap center")
		_expect(indices.size() == 72, "shared tracer mesh is not a closed twelve-sided taper")
...
func _finish() -> void:
	if _failures.is_empty():
		print("[AutocannonTracerSmoketest] PASS solid_closed_taper aspect_ratio<=4 lengths=0.8/1.1/1.35/1.75 cadence=profile_preserved")
		get_tree().quit(0)
		return
