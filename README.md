...
| Carrier Movement Tracking | Working | Parked, transport, catapult, and explicitly staged deck objects move with the scripted carrier; live wheel contact uses explicit carrier-relative velocity through `VelocityFrame` so aircraft and helicopters do not double-count carrier motion while touching down or rolling |
