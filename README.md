...
The live CSV is written to `user://airplane_aero_report.log` and mirrored to `airplane_aero_report.log` in the project. The launcher copies the fresh file to `captures/perf_play_sessions/<timestamp>/player_flight.csv`; if L3 markers exist, it also writes `player_flight_marks.csv` containing just those rows and records the marker count in `session.txt`. At roughly 600-900 bytes per row, 10 Hz produces about 0.35-0.55 MB per minute per copy. One-second buffering reduces normal recorder file opens from approximately 20 per second to two per second across the user and project copies; a marker deliberately forces an extra flush.
